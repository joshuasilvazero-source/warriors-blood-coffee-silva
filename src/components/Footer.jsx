export default function Footer() {
    return (
        <footer className="bg-brandBlack pt-20 pb-10 border-t border-white/5">
            <div className="max-w-7xl mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
                    {/* Brand Column */}
                    <div>
                        <h4 className="font-heading text-2xl mb-6">
                            Warriors <span className="text-brandRed">Blood</span> Coffee
                        </h4>
                        <p className="text-gray-400 text-sm leading-relaxed max-w-xs mb-6">
                            Premium coffee beans, roasted to order and shipped fresh from our
                            roastery to your cup since 2018.
                        </p>
                        <div className="flex space-x-4">
                            <a
                                href="https://instagram.com/yourhandle"
                                className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-brandGold transition-colors">
                                <span className="sr-only">Instagram</span>
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691
                                4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069
                                4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07
                                -3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058
                                -1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664
                                -4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069z"
                                    />
                                </svg>
                            </a>
                            <a
                                href="https://facebook.com/yourhandle"
                                className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-brandGold transition-colors">
                                <span className="sr-only">Facebook</span>
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99
                                    4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007
                                    1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83
                                    c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385
                                    C19.612 23.027 24 18.062 24 12.073z"
                                    />
                                </svg>
                            </a>
                        </div>
                    </div>

                    {/* Navigation Column */}
                    <div className="flex flex-col md:items-start">
                        <h4 className="font-heading text-brandGold tracking-widest uppercase mb-6 text-sm">
                            Navigation
                        </h4>
                        <ul className="space-y-4 text-sm font-semibold uppercase tracking-tighter">
                            {["Home", "Shop", "About Us", "Contact Us"].map((item) => (
                                <li key={item}>
                                    <a className="hover:text-brandGold transition-colors" href="#">
                                        {item}
                                    </a>
                                </li>
                            ))}
                        </ul>
                    </div>

                    {/* Contact Column */}
                    <div className="flex flex-col md:items-start text-sm">
                        <h4 className="font-heading text-brandGold tracking-widest uppercase mb-6">
                            Contact Us
                        </h4>
                        <address className="not-italic text-gray-400 space-y-4">
                            <p>Lawton, OK 73507</p>
                            <p>Phone: 1800-887-5309</p>
                            <p>Email: support@warriorsbloodcoffee.com</p>
                            <p>Website: warriorsbloodcoffee.com</p>
                        </address>
                    </div>
                </div>

                {/* Bottom Bar */}
                <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center text-[10px] uppercase tracking-widest text-gray-500">
                    <p className="text-center md:text-left">
                        &#169; 2026 Warriors Blood Coffee Co., LLC. All rights reserved.
                    </p>
                    <div className="flex space-x-6 mt-4 md:mt-0">
                        <a className="hover:text-brandGold" href="#">
                            Privacy Policy
                        </a>
                        <a className="hover:text-brandGold" href="#">
                            Terms of Service
                        </a>
                    </div>
                </div>
            </div>
        </footer>
    );
}
